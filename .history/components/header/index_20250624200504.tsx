...
export default function Header({ headerData }: HeaderProps) {
  ...
  const navigationLinks: NavItem[] = headerData?.navigationLinks?.map(link => {
    // If submenu items exist, force linkType to "none"
    const hasSubmenu = link.submenuItems && link.submenuItems.length > 0;
    const effectiveLinkType = hasSubmenu ? "none" : (link.linkType as NavItem["linkType"]) || "manual";
    
    return {
      label: link.label || "",
      linkType: effectiveLinkType,
      href: effectiveLinkType === "manual" ? link.href || "#" : resolveUrl(link),
    page: link.page ? {
